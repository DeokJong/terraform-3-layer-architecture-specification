# ADR: 3-Layer Architecture and 계약 / 워크스페이스 Model

## 제목

3-Layer Architecture에서 Layer, 계약, 워크스페이스, Resource Set 규칙 정의
...
현재 인프라는 여러 서비스와 공유 리소스가 혼재된 상태로 운영되고 있으며, 레이어 간 참조, 공유 리소스 ownership, SSM/Route53/Secret 같은 게시 surface의 책임, Terraform 워크스페이스 분리 기준이 일관되게 정리되어 있지 않습니다.

또한 일부 환경은 모놀로식 Terraform 워크스페이스에 과도하게 많은 리소스가 집약되어 있습니다. 이 구조에서는 한 환경에 대한 변경 PR이 올라와 Terraform lock이 잡히면 다른 PR은 같은 환경에 대해 Plan조차 수행하지 못합니다. 그 결과 변경 검증이 직렬화되고, 리뷰와 배포 대기 시간이 길어집니다.
...
반면 Lambda 계열은 이미 Serverless Framework Stack으로 분리 관리되고 있어, Terraform이 관리하는 주제와 별도 배포 단위가 혼재되어 있습니다. 즉 현재 상태는 "모든 것을 한 워크스페이스에 둔 단순한 구조"도 아니고, 그렇다고 ownership과 운영 경계를 기준으로 정리된 구조도 아닙니다.

이로 인해 다음 문제가 반복됩니다.

- 상위 레이어 변경이 과도하게 하위 레이어 변경으로 전파된다.
- 공유 리소스와 서비스 전용 리소스의 경계가 불명확하다.
- SSM, Route53 Record, Secret 같은 게시 매체와 ownership이 혼동된다.
- Terraform 워크스페이스가 지나치게 비대해지거나, 반대로 변경 경계 없이 쪼개져 영향 범위가 커진다.
...
참조 방향은 `Foundation -> Platform -> Service` 순서로만 허용한다. 의미상으로는 하위 레이어가 상위 레이어가 게시한 계약를 소비하는 구조를 따른다.

### 2. 워크스페이스는 Layer와 별개의 운영 단위다

워크스페이스는 Terraform state, apply, 권한, 영향 범위를 제어하기 위한 운영 단위다. Layer와 워크스페이스는 동일 개념이 아니며 1:1 대응을 강제하지 않는다.

같은 Layer 안에서도 다음 특성에 따라 여러 워크스페이스로 분리할 수 있다.
...
### 3. 모든 레이어 전달값은 계약로 간주한다

하위 레이어는 상위 레이어의 implementation value를 직접 참조하지 않고, 상위 레이어가 게시한 계약만 사용한다.
...
### 4. 계약 ownership은 provider에게 있다

계약 ownership은 누가 읽는지, 어디에 저장되는지, 어떤 매체로 전달되는지로 결정하지 않는다.

계약 ownership은 그 계약가 표현하는 리소스 또는 인터페이스를 누가 제공하고, 그 의미와 안정성을 누가 보장하는지로 결정한다. 즉 consumer가 아니라 provider가 owner다.

### 5. 구현값와 계약 Value를 구분한다

하위 레이어는 상위 레이어의 구현값를 직접 참조하지 않는다.

- 구현값: 내부 구현 세부사항
- 계약 Value: consumer가 소비해도 되는 안정된 값

provider는 가능한 한 stable DNS, standard output, standard parameter path 같은 계약 Value를 게시해야 한다.
...
Resource Set 내부 구성요소는 영향 범위, 변경 빈도, ownership, rollback 경계에 따라 선택적으로 분리할 수 있다.

대표적인 내부 분리 패턴은 다음과 같다.

- Primary Resource: 본체 리소스
- Binding: allowlist, attachment, grant, policy 바인딩
...
- 워크스페이스를 영향 범위와 운영 경계에 맞게 분리할 수 있다.
- Service 간 직접 참조를 줄이고 계약 중심 구조로 전환할 수 있다.
...
- 바인딩 workspace, 게시 workspace 분리로 workspace 수가 증가할 수 있다.
...
### 대안 2. Layer와 워크스페이스를 1:1로 강제한다

설명은 쉬우나 실제 운영에서는 변경 특성이 다른 구성요소를 분리하기 어려워 영향 범위가 커진다.

### 대안 3. 계약 ownership을 저장 위치 기준으로 정한다

SSM, Route53, Secret 같은 게시 매체와 ownership이 혼동되어 잘못된 해석이 반복된다.

## 적용 기준

새 리소스 또는 계약 추가 시 아래 순서로 판단한다.
...
- Redis ingress allowlist는 Platform 바인딩 workspace가 관리한다.
