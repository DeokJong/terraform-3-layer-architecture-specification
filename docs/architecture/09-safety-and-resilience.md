...
- 공유 리소스 변경이 불안정하게 큰 영향 범위를 만들지 않을 것
...
- implementation change와 계약 변경을 분리한다.
- Active 계약은 마이그레이션 없이 직접 바꾸지 않는다.
- breaking change는 병행 게시와 마이그레이션을 통해 수행한다.

### 장애 격리

- 공유 primary resource와 바인딩 churn은 필요 시 분리한다.
...
- 계약 마이그레이션은 병행 운영이 가능해야 한다.
- legacy 자산은 즉시 rename보다 안전한 전환을 우선한다.

## 안전성 설계 규칙

- 영향 범위가 다른 리소스를 무조건 같은 워크스페이스에 넣지 않는다.
- access rule 변경이 잦은 공유 리소스는 primary resource와 분리 검토한다.
- 게시는 Consumer 안정성을 높이는 방향으로 설계한다.
...
- 마이그레이션 없이 직접 교체하려는 Active 계약이 있는가
- source of truth와 게시가 혼동되고 있지 않은가
