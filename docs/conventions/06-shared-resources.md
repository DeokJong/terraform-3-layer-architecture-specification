...
shared resource는 먼저 하나의 리소스 세트로 정의합니다. 리소스 세트는 하나의 명확한 shared capability를 제공하는 리소스 묶음입니다.
...
문서와 설계 검토에서는 가능하면 shared resource를 먼저 리소스 세트로 설명합니다.

## 내부 분리 판단 기준

리소스 세트 내부는 다음 기준이 있을 때 분리를 검토합니다.
...
위 조건이 약하면 하나의 리소스 세트 또는 하나의 workspace로 유지할 수 있습니다.

## Core / Access / Publication은 선택 패턴

`Core / Access / Publication`은 리소스 세트 내부를 설명하는 대표 패턴으로 사용합니다.
