...
| Provider | 계약를 제공하고 의미를 보장하는 주체 |
| Consumer | 계약를 읽거나 사용하는 주체 |
| 계약 | 레이어 간 의존을 위해 공식적으로 게시된 값 또는 인터페이스 |
| 구현값 | 내부 구현 세부사항으로 직접 의존하면 안 되는 값 |
| 계약 Value | consumer가 의존해도 되는 안정된 값 |
| Core | shared resource의 본체 리소스 |
| Access | allowlist, grant, 바인딩, policy 같은 접근 제어 영역 |
...
### 3.3 하위 레이어는 계약만 소비한다

하위 레이어는 상위 레이어의 implementation value를 직접 참조하지 않고, 상위 레이어가 게시한 계약만 사용해야 합니다.

### 3.4 계약 ownership은 provider에게 있다

계약의 owner는 값을 읽는 consumer가 아니라, 해당 값을 제공하고 의미와 안정성을 보장하는 provider입니다.

### 3.5 워크스페이스는 운영 단위다

워크스페이스는 state, apply, 권한, 변경 전파 범위, 영향 범위를 제어하기 위한 운영 단위입니다. Layer와 워크스페이스는 동일 개념이 아닙니다.

### 3.6 shared resource는 Resource Set으로 설명 가능해야 한다

shared resource는 우선 하나의 shared capability를 제공하는 Resource Set으로 설명할 수 있어야 합니다.
...
중요한 점은 내부 구조를 처음부터 고정 역할로 강제하지 않는다는 것입니다. 초기 규모가 작고 lifecycle, 변경 주기, 운영 주체가 같다면 하나의 워크스페이스로 함께 관리할 수 있습니다. 다만 변경 churn과 영향 범위가 커질 때는 내부를 선택적으로 분리할 수 있어야 합니다.
...
위 다이어그램은 "참조 가능한 방향"을 표현합니다. 의미상으로는 하위 레이어가 상위 레이어가 게시한 계약를 소비하는 구조입니다.
...
이 뷰는 Layer와 워크스페이스가 같은 개념이 아니라는 점을 보여주기 위한 예시입니다. 하나의 Layer 안에서도 capability 단위의 Resource Set과 게시 workspace가 함께 존재할 수 있습니다.
...
계약는 레이어 간 의존성을 표현하기 위해 공식적으로 게시된 값 또는 인터페이스입니다.
...
계약는 단순한 값 목록이 아니라 아래 성질을 가져야 합니다.
...
즉, 어떤 값이 외부에 노출되었다고 해서 자동으로 계약가 되는 것은 아닙니다. provider가 공식 surface로 관리하는 값만 계약로 취급합니다.
...
위 질문에 그렇다고 답하기 어렵다면 구현값일 가능성이 큽니다.
...
중요한 점은 게시 surface가 ownership을 결정하지 않는다는 것입니다. 예를 들어 SSM에 저장되었더라도 그 값이 Platform이 제공하는 DB endpoint contract라면 owner는 Platform입니다.

### 6.5 계약 owner와 consumer의 책임
...
아키텍처 문서 또는 module 문서에 계약를 명시할 때 최소한 아래 항목을 포함합니다.
...
### 6.7 계약 source of truth

게시 workspace나 aggregation workspace가 존재하더라도 source of truth는 원 provider에 남아 있어야 합니다.
...
- Removed는 모든 consumer 마이그레이션 완료 후 제거된 상태다.
...
- `db-main.internal.example.com`: Connectivity 계약
- `/platform/db/main/host`: Runtime 계약

backend 서비스는 위 둘 중 provider가 공식화한 계약만 사용해야 하며, 직접 RDS endpoint를 참조하지 않습니다.

#### 사례 2. shared bucket

- bucket physical ID: 구현값일 수 있음
- stable bucket name: Identity 계약
- bucket consumer principal 바인딩: Access 계약

#### 사례 3. service API

- internal ALB DNS: 구현값
- `orders-api.internal.example.com`: Service가 제공하는 Connectivity 계약
...
- temporary 마이그레이션 output을 영구 인터페이스처럼 사용하는 것
...
다음 조건을 만족하면 같은 워크스페이스에 둘 수 있습니다.
...
예를 들어 하나의 shared resource에 대해 Resource Set 내부 구성요소가 모두 같은 팀에 의해 함께 관리되고, consumer 수도 적고, access 변경도 드물다면 하나의 워크스페이스로 관리할 수 있습니다.

예:

- 하나의 `user-db`에 대해 DB cluster, SG allowlist, SSM 게시을 함께 관리
- 하나의 `shared-storage`에 대해 bucket, policy 바인딩, output 게시을 함께 관리
- 하나의 `shared-cache`에 대해 cluster, ingress allowlist, endpoint 게시을 함께 관리
...
- 고위험 구성요소와 churn이 높은 바인딩이 섞여 있다.
- 서비스 온보딩이나 access rule 변경이 잦다.
- 게시 contract를 별도 마이그레이션해야 할 가능성이 크다.

### 8.4 워크스페이스와 계약의 관계

워크스페이스는 계약를 게시할 수 있지만 계약 owner와 항상 동일하지는 않습니다.

예:

- `user-db` Resource Set이 DB를 생성하고 lifecycle을 관리한다.
- `user-db-게시` workspace가 stable endpoint를 SSM이나 DNS에 게시한다.
- 이 경우 계약 owner는 여전히 `user-db` Platform domain이며, 게시 workspace는 게시 책임만 수행한다.

즉, 게시 workspace는 source of truth를 대체하지 않습니다.

### 8.5 워크스페이스 anti-pattern

- 변경 특성이 뚜렷이 다른데도 하나의 workspace가 고위험 resource body와 churn이 높은 바인딩, 게시을 모두 관리
...
### 8.7 워크스페이스 분리 판단 매트릭스

| 질문 | 예 | 답이 "예"면 |
| --- | --- | --- |
| 항상 함께 변경되는가 | `user-db` 본체와 게시이 항상 같이 수정됨 | 같은 workspace 가능 |
| ownership이 동일한가 | 동일 팀이 lifecycle 전체 관리 | 같은 workspace 가능 |
| access churn이 잦은가 | consumer onboarding이 자주 발생 | 바인딩 전용 분리 우선 검토 |
| rollback 위험이 다른가 | DB 교체와 policy 수정의 위험도 차이 큼 | 분리 검토 |
| consumer contract 마이그레이션이 필요한가 | DNS/SSM path 이행 중 | 게시 분리 검토 |
| 영향 범위가 크게 다른가 | 버킷 본체와 policy 바인딩 영향 범위 차이 큼 | 분리 검토 |
...
하나의 Resource Set 안에 모든 구성요소를 둘 수도 있고, 영향 범위와 lifecycle 차이에 따라 내부를 여러 workspace나 하위 set으로 나눌 수도 있습니다.
...
즉 shared resource의 1차 경계는 역할명이 아니라 capability boundary이며, 내부 분리는 영향 범위와 운영 경계에 따라 선택합니다.
...
소수 consumer만 사용하고 access churn이 낮으며 ownership이 동일하다면 하나의 workspace로 둘 수 있습니다. 반대로 consumer 수 증가, 잦은 allowlist 변경, contract 마이그레이션, 별도 KMS governance 필요성이 생기면 그 시점에 내부 분리를 검토합니다.
...
consumer는 shared resource를 직접 변경하는 대신 access contract와 게시 contract를 통해 연결되어야 합니다.

중요한 점은 게시 location이 ownership을 결정하지 않는다는 것입니다. 예를 들어 legacy 경로 아래 SSM parameter를 게시하더라도, 그 값의 의미와 lifecycle을 shared resource owner가 책임지면 owner는 여전히 Platform Resource Set입니다.
...
| `shared-storage` | bucket, encryption, policy 바인딩, published output | core, access, 게시 |
...
필요한 경우 provider 역할을 가진 서비스가 계약를 게시하고, consumer는 그 계약만 참조합니다.
...
위 네 가지는 모두 잘못된 판단이 될 수 있습니다. ownership과 분리 기준은 usage 수가 아니라 lifecycle, 의미, compatibility, 영향 범위로 판단해야 합니다.
...
즉시 rename보다 ownership 재해석과 consumer 마이그레이션이 우선입니다.
...
- shared resource 변경이 불필요하게 넓은 영향 범위를 만들지 않을 것
- consumer가 provider 내부 구현 변화에 직접 노출되지 않을 것
- 실패 시 rollback, isolate, 마이그레이션이 가능할 것
...
- Active 계약는 조용히 의미를 바꾸지 않는다.
- breaking change는 병행 게시와 마이그레이션을 통해 수행한다.
...
- service-to-service는 공식 계약만 사용한다.
- 게시 surface는 source of truth를 숨기거나 왜곡하지 않는다.

#### 복구 가능성

- workspace는 rollback 단위로도 의미가 있어야 한다.
- contract 마이그레이션은 병행 운영이 가능해야 한다.
- legacy 자산은 즉시 rename보다 안전한 전환을 우선한다.

### 12.3 안전성 설계 규칙

- 영향 범위가 다른 리소스를 무조건 같은 workspace에 넣지 않는다.
- access rule 변경이 잦은 shared resource는 primary resource와 분리 검토한다.
- 게시은 consumer 안정성을 높이는 방향으로 설계한다.
- provider는 contract compatibility를 명시적으로 책임진다.
- 마이그레이션 없는 in-place breaking change를 금지한다.
...
- 게시 surface만 보고 ownership을 판단하는 구조
- Active 계약를 마이그레이션 없이 교체하는 구조
...
| Is 게시 separate from core | Yes or no with reason |

### 16.3 Resource Set Partition Plan
...
| Binding Set | allowlist, 바인딩s, grants, policy attachments | provider or access owner | medium or high | split if churn is high |
| Publication Set | DNS, SSM, output 게시 | provider | low or medium | split if 마이그레이션 is likely |

### 16.4 계약 Record

| 계약 Name | Type | Consumer | Publication | Source of Truth | Stability |
| --- | --- | --- | --- | --- | --- |
...
- Can contract 마이그레이션 run in parallel without breaking consumers

## 17. Migration and Rollout Scenarios

### 17.1 계약 Migration

1. Identify the current Active 계약.
2. Define and publish the new 계약 in parallel.
3. Identify consumers and 마이그레이션 order.
4. Operate both contracts during 마이그레이션.
5. Move the old contract to Deprecated after 마이그레이션.
...
3. Check whether the existing 계약 can continue to represent the new core.
4. If yes, switch 게시 while preserving contract meaning.
5. If not, publish a new 계약 in parallel and migrate consumers.
